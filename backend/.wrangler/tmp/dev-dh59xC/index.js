...
async function signJWT(serviceAccount) {
  ...
  const message2 = `${header}.${payload}`;
  const encoder2 = new TextEncoder();
  const data = encoder2.encode(message2);
  ...
  return `${message2}.${signatureBase64}`;
}
...
function getAdmin(env2) {
  ...
  const mapDoc = /* @__PURE__ */ __name((doc) => {
    ...
    for (const [key, val] of Object.entries(fields)) {
      if (val.stringValue !== void 0) res[key] = val.stringValue;
      else if (val.integerValue !== void 0) res[key] = parseInt(val.integerValue, 10);
      else if (val.doubleValue !== void 0) res[key] = parseFloat(val.doubleValue);
      else if (val.booleanValue !== void 0) res[key] = val.booleanValue;
      else if (val.timestampValue !== void 0) res[key] = val.timestampValue;
      else if (val.mapValue !== void 0) res[key] = val.mapValue.fields;
      else if (val.arrayValue !== void 0) res[key] = (val.arrayValue.values || []).map((v) => Object.values(v)[0]);
      else res[key] = val;
    }
    return res;
  }, "mapDoc");
...
// .wrangler/tmp/bundle-yv8LmV/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yv8LmV/middleware-loader.entry.ts
