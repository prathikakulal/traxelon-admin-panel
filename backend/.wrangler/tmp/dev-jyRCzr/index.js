...
async function signJWT(serviceAccount) {
  ...
  const pemContents = serviceAccount.private_key.replace(/-----BEGIN PRIVATE KEY-----/g, "").replace(/-----END PRIVATE KEY-----/g, "").replace(/\s/g, "");
...
async function getAccessToken(env2) {
  ...
  const b64 = env2.FIREBASE_SERVICE_ACCOUNT_B64 || process.env.FIREBASE_SERVICE_ACCOUNT_B64;
  ...
  accessTokenCache = {
    ...
    expiry: Date.now() + (parseInt(data.expires_in || 3600, 10) - 60) * 1e3
  };
  return data.access_token;
}
__name(getAccessToken, "getAccessToken");
function getAdmin(env2) {
  const projectId = env2.FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID || "traxelon-final";
  const baseUrl = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents`;
  const request = /* @__PURE__ */ __name(async (path, options = {}) => {
  ...
  const mapValue = /* @__PURE__ */ __name((v) => {
    if (!v) return null;
    if (v.stringValue !== void 0) return v.stringValue;
    if (v.integerValue !== void 0) return parseInt(v.integerValue, 10);
    if (v.doubleValue !== void 0) return parseFloat(v.doubleValue);
    if (v.booleanValue !== void 0) return v.booleanValue;
    if (v.timestampValue !== void 0) return v.timestampValue;
    if (v.nullValue !== void 0) return null;
    if (v.mapValue !== void 0) {
    ...
    if (v.arrayValue !== void 0) {
      return (v.arrayValue.values || []).map(mapValue);
    }
    return v;
  }, "mapValue");
  const mapDoc = /* @__PURE__ */ __name((doc) => {
  ...
  const toValue = /* @__PURE__ */ __name((v) => {
  ...
  return {
    firestore: /* @__PURE__ */ __name(() => ({
      collection: /* @__PURE__ */ __name((col) => ({
        doc: /* @__PURE__ */ __name((id) => ({
          get: /* @__PURE__ */ __name(async () => {
            const d = await request(`${col}/${id}`);
            if (d.error) return { exists: false, data: /* @__PURE__ */ __name(() => ({}), "data") };
            return { exists: !!d.name, data: /* @__PURE__ */ __name(() => mapDoc(d), "data") };
          }, "get"),
          update: /* @__PURE__ */ __name(async (dataProps) => {
            const fields = {};
            const updateMask = [];
          ...
          set: /* @__PURE__ */ __name(async (dataProps) => {
            const fields = {};
            for (const [k, v] of Object.entries(dataProps)) {
              fields[k] = toValue(v);
            }
          ...
          delete: /* @__PURE__ */ __name(() => request(`${col}/${id}`, { method: "DELETE" }), "delete"),
          collection: /* @__PURE__ */ __name((sub) => ({
            get: /* @__PURE__ */ __name(async () => {
              const data = await request(`${col}/${id}/${sub}?pageSize=100`);
              return { docs: (data.documents || []).map((d) => ({ id: d.name.split("/").pop(), data: /* @__PURE__ */ __name(() => mapDoc(d), "data") })) };
            }, "get")
          }), "collection")
        }), "doc"),
        get: /* @__PURE__ */ __name(async () => {
      ...
      batch: /* @__PURE__ */ __name(() => ({ commit: /* @__PURE__ */ __name(() => {
      }, "commit") }), "batch")
    }), "firestore"),
    auth: /* @__PURE__ */ __name(() => ({
      deleteUser: /* @__PURE__ */ __name(async (uid) => {
        const token = await getAccessToken(env2);
    ...
    FieldValue: { increment: /* @__PURE__ */ __name((n) => ({ integerValue: n }), "increment"), delete: /* @__PURE__ */ __name(() => null, "delete") }
  };
}
...
router.get("/links/:id/captures", async (c) => {
  try {
    ...
    try {
      const capsSnap = await db.collection("trackingLinks").doc(linkId).collection("captures").get();
      const subCaps = capsSnap.docs.map((d) => normaliseCapture({ id: d.id, ...d.data() }));
      const existingIds = new Set(captures.map((c2) => c2.id).filter(Boolean));
      subCaps.forEach((sc) => {
        if (!existingIds.has(sc.id)) captures.push(sc);
      });
    } catch (err) {
...
app.get("/t/:id", async (c) => {
  ...
  try {
    ...
    await docRef.update({
      ...
      lastClickAt: (/* @__PURE__ */ new Date()).toISOString()
    });
    return c.html(`
      <!DOCTYPE html>
      <html>
...
app.post("/api/capture/:id", async (c) => {
  ...
  try {
    ...
    const getBrowser = /* @__PURE__ */ __name((u) => {
    ...
    const newCapture = {
      ...clientData,
      // Preserve all 40+ attributes sent by frontend
      ...
      capturedAt: clientData.capturedAt || (/* @__PURE__ */ new Date()).toISOString()
    };
    await docRef.update({
...
// .wrangler/tmp/bundle-eKFw1t/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-eKFw1t/middleware-loader.entry.ts
