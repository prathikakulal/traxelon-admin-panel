...
// ── GET /api/admin/links/:id/captures ────────────────────────────────────────
router.get('/links/:id/captures', async (c) => {
  try {
    const linkId = c.req.param('id')
    const admin = getAdmin(c.env)
...
// Helper: random 6-char segment (no O/0, I/1 confusion)
...
function buildVoucherCode(prefix, middleName, suffix) {
  ...
  return [prefix, middleName, rand, suffix]
    .filter(Boolean)
    .map(s => s.toUpperCase())
    .join('-')
}
...
// Body: { prefix, middleName, suffix, startDate, endDate, credits }
router.post('/vouchers', async (c) => {
  try {
    ...
    // Use a random doc ID since this REST-based admin has no .add()
