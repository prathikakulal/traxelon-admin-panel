...
// //       // ✅ No manual setOfficers/setStats here — onSnapshot listeners handle UI updates
...
// //       // Also update dashboardStats so overview reflects the change via its own onSnapshot
// //       try { await updateDoc(doc(db, 'metadata', 'dashboardStats'), { totalCredits: increment(amount) }) } catch(err) {}
// //       // ✅ No manual setOfficers/setStats — onSnapshot handles UI
...
// //   // ── Real-time listener for trackingLinks ──
// //   useEffect(() => {
// //     if (!authed) return
...
// //       setLinks(prev => {
// //         // Merge by id: keep older pages not covered by snapshot + fresh snapshot docs
// //         const snapshotIds = new Set(docs.map(d => d.id))
// //         const olderPages = prev.filter(r => !snapshotIds.has(r.id))
// //         return [...docs, ...olderPages]
// //       })
// //     }, (err) => {
...
// //       setOfficers(prev => {
// //         // Merge by uid: keep older pages not covered by snapshot + fresh snapshot docs
// //         const snapshotIds = new Set(docs.map(d => d.uid))
// //         const olderPages = prev.filter(r => !snapshotIds.has(r.uid))
// //         return [...docs, ...olderPages]
// //       })
// //     }, (err) => {
...
import ActivityView from './ActivityView.jsx'
import PaymentsView from './PaymentsView.jsx'
import VouchersView from './VouchersView.jsx'
import { P, STYLES } from '../styles/theme.js'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (!authed) return
    const q = query(collection(db, 'trackingLinks'), orderBy('createdAt', 'desc'), limit(20))
    const unsub = onSnapshot(q, (snap) => {
      const docs = snap.docs.map(d => {
        const data = d.data()
        return { id: d.id, ...data, token: data.token || d.id }
      })
      setLinks(prev => {
        const snapshotIds = new Set(docs.map(d => d.id))
        const olderPages = prev.filter(r => !snapshotIds.has(r.id))
  ...
  useEffect(() => {
    ...
    }, (err) => { console.error('[DEBUG] Users Real-time Error:', err.message) })
    return () => unsub()
  }, [authed])
  ...
  const views = {
    ...
    credits:  <CreditsView officers={officers} onAddCredit={handleAddCredit} onDeductCredit={handleDeductCredit} onDelete={handleDelete} />,
    coupons:  <CouponsView showToast={showToast} />,
    vouchers: <VouchersView showToast={showToast} />,
    activity: <ActivityView />,
    payments: <PaymentsView showToast={showToast} officers={officers} onAddCredit={handleAddCredit} />,
  }
      ...
      <div className="layout-wrapper" style={{
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <main style={{ flex: 1, overflow: 'auto', padding: 22 }}>
            ...
            {fetchError && (
              <div style={{ marginBottom: 18, padding: '12px 16px', background: 'rgba(255,59,48,.08)', border: '1px solid rgba(255,59,48,.3)', borderRadius: 10, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <AlertTriangle size={15} color="#ff3b30" style={{ flexShrink: 0, marginTop: 1 }} />
                <div>
                  <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 13, color: '#ff3b30', fontWeight: 600, marginBottom: 3 }}>
                    Could not load data from database
