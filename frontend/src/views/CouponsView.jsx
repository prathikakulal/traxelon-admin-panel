...
export default function CouponsView({ showToast }) {
  ...
  const handleCreate = async () => {
    if (!code.trim() || !discount || parseInt(discount) < 1) {
      showToast('Enter a valid code and discount percentage', false); return
    }
    setLoading(true)
    try {
      await addDoc(collection(db, 'coupons'), {
      ...
      setCode(''); setDiscount(''); setExpiry(''); setMaxUses('')
      showToast('Coupon created successfully ✓')
    } catch (e) { showToast(e.message, false) }
    setLoading(false)
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div className="atc" style={{ padding: 22 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12, marginBottom: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 10, color: P.muted, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, marginBottom: 6, textTransform: 'uppercase' }}>Discount (%)</div>
            <input
              type="number"
              min="1"
              max="98"
              ...
              {coupons.map(c => {
                ...
                return (
                  <tr key={c.id} className="atr" style={{ borderBottom: `1px solid ${P.border}18`, backgroundColor: isSelected ? `${P.cyan}10` : 'transparent' }}>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 16, color: P.yellow, fontWeight: 700 }}>
                        {c.discount ?? c.credits ?? '—'}
                      </span>
                      <span style={{ fontSize: 10, color: P.muted, marginLeft: 4 }}>%</span>
                    </td>
                    <td style={{ padding: '12px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: P.txt2 }}>
                      {c.usedCount || 0} / {c.maxUses >= 999 ? '∞' : c.maxUses}
                    </td>
