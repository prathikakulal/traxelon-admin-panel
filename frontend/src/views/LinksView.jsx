...
const LV_CSS = `
  ...
  /* ── Link header row: stack on mobile, row on desktop ── */
  .lv-header-row {
    padding: 14px 14px;
    display: flex;
  ...
  /* ── CaptureCard header: stack on mobile ── */
  .cc-header-row {
    padding: 14px;
    cursor: pointer;
    display: flex;
...
const CaptureCard = memo(({ c, i }) => {
  ...
  return (
    <div className="capture-wrapper" style={{ 
      ...
      {/* ── Mobile header ── */}
      <div
        className="cc-header-row cc-mobile-only"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <div className="cc-mobile-top">
          <div style={{ 
            background: 'rgba(0,212,255,0.08)', color: P.cyan, fontSize: 10, fontWeight: 700,
            padding: '3px 7px', borderRadius: 4, border: `1px solid ${P.cyan}33`,
            fontFamily: "'JetBrains Mono',monospace", flexShrink: 0
          }}>#{i}</div>
          <div style={{ 
            display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(255,59,48,0.08)', 
            color: P.red, fontSize: 10, fontWeight: 700, padding: '3px 7px', borderRadius: 20,
            border: `1px solid ${P.red}44`, flexShrink: 0
          }}>
      ...
      {isExpanded && (
        <div style={{ 
          padding: '0 16px 20px 16px', animation: 'fadeIn 0.3s ease-out',
          display: 'flex', flexDirection: 'column', gap: 18
        }}>
          <div style={{ borderTop: `1px solid ${P.border}33`, paddingTop: 18 }} />
