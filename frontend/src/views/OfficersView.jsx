...
<body>
  ...
  const win = window.open('', '_blank', 'width=1100,height=800')
  if (win) {
    // Inject a floating print button into the report — no auto print dialog
    const htmlWithPrintBtn = html.replace(
      '<body>',
      `<body>
      <div style="position:fixed;top:16px;right:16px;z-index:9999;display:flex;gap:8px;">
        <button onclick="window.print()" style="background:linear-gradient(135deg,#00d4ff,#0088aa);color:#0a0f1e;border:none;padding:9px 20px;border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;letter-spacing:1px;cursor:pointer;">
          PRINT / SAVE PDF
        </button>
        <button onclick="window.close()" style="background:#1e293b;color:#94a3b8;border:1px solid #334155;padding:9px 16px;border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:12px;cursor:pointer;">
          CLOSE
        </button>
      </div>`
    )
